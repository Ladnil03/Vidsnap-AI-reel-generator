...
void main() {
  group('CreatorRepository Unit Tests', () {
    ...
    test(
      'updateProfile sends PUT and returns updated CreatorProfileModel',
      () async {
        dio.httpClientAdapter = _MockAdapter((options) {
          expect(options.path, '/api/v1/creator/profile');
    ...
    test(
      'getAnalytics returns audience metrics, watch time, and affinity',
      () async {
        dio.httpClientAdapter = _MockAdapter((options) {
    ...
    test(
      'getCopilotInsights sends prompt and returns AI hooks with viral score',
      () async {
        dio.httpClientAdapter = _MockAdapter((options) {
          ...
          final data = {
            ...
            'hooks': [
              {
                'hook_text':
                    'Stop writing slow mobile apps with this new trick',
        ...
        final copilot = await repository.getCopilotInsights(
          topic: 'Flutter 3.47',
        );
