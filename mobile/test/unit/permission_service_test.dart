...
void main() {
  group('PermissionService Unit Tests', () {
    ...
    test('metadata provides clear rationale and icons for camera, mic, photos and storage', () {
      expect(
        PermissionService.metadata.containsKey(AppPermission.camera),
        isTrue,
      );
      expect(
        PermissionService.metadata[AppPermission.camera]?.title,
        'Camera Access',
      );
      expect(
        PermissionService.metadata[AppPermission.microphone]?.title,
        'Microphone Access',
      );
      expect(
        PermissionService.metadata[AppPermission.photos]?.title,
        'Photo & Video Library',
      );
      expect(
        PermissionService.metadata[AppPermission.storage]?.title,
        'Storage Access',
      );
    });

    test(
      'initial permission state defaults to denied before request',
      () async {
    ...
    testWidgets(
      'shows educational rationale dialog when requesting permission and grants on accept',
      (tester) async {
        ...
        expect(
          find.textContaining('VidSnap.AI requires camera access'),
          findsOneWidget,
        );
        ...
        expect(
          service.getPermissionState(AppPermission.camera),
    ...
    testWidgets(
      'educational rationale dialog denies permission when user taps Not Now',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: Builder(
        ...
        expect(
          service.getPermissionState(AppPermission.microphone),
