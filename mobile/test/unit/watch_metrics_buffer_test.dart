...
class _FakeFeedRepository implements FeedRepository {
  final List<WatchProgressRequestModel> recordedBeacons =
      <WatchProgressRequestModel>[];

  @override
  dynamic noSuchMethod(Invocation invocation) {
    if (invocation.memberName == #recordWatchProgress) {
      final req =
          invocation.positionalArguments.first as WatchProgressRequestModel;
...
void main() {
  group('WatchMetricsBuffer Unit Tests', () {
    ...
    test(
      'recordProgress updates pending beacon and flushes to repository',
      () async {
        ...
        final beacon1 = fakeRepository.recordedBeacons.firstWhere(
          (b) => b.videoId == 'vid-10',
        );
        expect(beacon1.watchedSeconds, 15.0);
        expect(beacon1.completed, false);

        final beacon2 = fakeRepository.recordedBeacons.firstWhere(
          (b) => b.videoId == 'vid-20',
        );
