...
void main() {
  group('GlobalErrorBoundary Widget Tests', () {
    ...
    testWidgets('renders child normally when no error is present', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: GlobalErrorBoundary(
            child: Text('Normal Application Content'),
          ),
        ),
      );
    ...
    testWidgets('displays graceful fallback when error is reported via context and allows retry', (tester) async {
      ...
      expect(find.textContaining('An unexpected issue occurred'), findsOneWidget);
